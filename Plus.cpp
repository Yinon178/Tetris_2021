...
//move
bool Plus::move(char keyPressed)
{
	bool flag = true;
	...
	case VERTICAL: // --------------------------------------- VERTICAL -----------------------------------------
		...
		{
		case eKEYS::DEFAULT:
			if (boardGame.isValid(body[3].getx(), body[3].gety() + 1) && boardGame.isValid(body[1].getx(), body[1].gety() + 1))
			{
				//TURN OFF
				for (int i = 0; i < SIZE; i++)
					boardGame.turnOffPoint(body[i].getx(), body[i].gety());
	...
	case UPSIDE_DOWN: // --------------------------------------- UPSIDE_DOWN -----------------------------------------
		...
		case eKEYS::DEFAULT:
			for (int i = 0; i < SIZE; i++)
			{
				if ((i != 2) && boardGame.isValid(body[i].getx(), body[i].gety() + 1) == false)
				{
					while (boardGame.cleanLines(body[i].gety() + 1)) {
						boardGame.updateBoard();
					}
...
void Plus::rotate()
{
	if (adjust == HORIZON)
	{
		body[0].setPoint(body[1].getx(), body[1].gety(), true, getSerialObj());
